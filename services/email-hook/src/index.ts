...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/notify/friend-request-received') {
      return handleFriendRequestReceived(request, env)
    }

    if (url.pathname === '/notify/friend-request-responded') {
      return handleFriendRequestResponded(request, env)
    }

    // Default: Supabase Auth email hook
    return handleEmailHook(request, env)
  },
}
